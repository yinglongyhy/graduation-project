...
export default {
  ...
  data() {
    return {
      ...
      params: {},
      url: '/api/admin/picture2houseInfo/page',
      list: null,
      listLoading: true
    }
  },
  ...
  methods: {
    ...
    update() {
      ...
      editById('/api/admin/picture2houseInfo', this.form).then((response) => {
        row.id = this.form.id
        row.picture = this.form.picture
    ...
    handleDelete(index, row) {
      deleteById('/api/admin/picture2houseInfo/' + row.id).then((response) => {
        this.$refs.page.refresh()
      })
    }
  }
