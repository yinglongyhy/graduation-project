<template>
  <div class="app-container">
    ...
    <el-dialog title="编辑" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        <el-form-item label="租客编号" :label-width="formLabelWidth">
          <el-input v-model="form.tenant" autocomplete="off" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    update() {
      ...
      editById('/api/admin/lease', this.form).then((response) => {
        row.id = this.form.id
        row.tenant = this.form.tenant
    ...
    handleDelete(index, row) {
      deleteById('/api/admin/lease/' + row.id).then((response) => {
        this.$refs.page.refresh()
      })
    }
  }
